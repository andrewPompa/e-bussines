...
class AuthorizationInterceptor @Inject()(val mat: Materializer, router: Router, authenticationService: AuthenticationService)  ( ec: ExecutionContext) extends Filter {
        ...
        if (!authorizationRequired(request)) {
            ...
            getUserResponse.flatMap{ userOption =>
                if (userOption.isEmpty) {
                   Future{ Unauthorized("user unauthorized!") }
                } else {
                    next(request)
                }
            }
