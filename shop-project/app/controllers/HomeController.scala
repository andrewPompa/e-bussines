...
class HomeController @Inject()(productsRepo: ProductRepository, categoryRepo: CategoryRepository,
    ...
    productForm.bindFromRequest.fold(
      ...
      product => {
        productsRepo.create(product.name, product.description, product.category).map { _ =>
          ...
          Redirect(routes.HomeController.index).flashing("success" -> "product.created")
        }
      }
    )
  }
